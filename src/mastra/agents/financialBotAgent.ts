...
2. **Понятные сообщения** - Пиши простым языком для студентов
3. **Быстрая оплата** - Оплата ПЕРЕД заполнением опроса
4. **Логирование** - Все важные действия логируй через logger
5. **Обработка ошибок** - Всегда проверяй результаты вызовов инструментов
